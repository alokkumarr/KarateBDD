Feature: Kafka Test Suite 22
  ...
  Scenario: Creating Consumer
    ...
    Then status 200
    * print response
    * match response.instance_id == "trainer_consumer"
  ...
  Scenario: Subscribing to topic
    Given path "/consumers/my-consumer-group/instances/trainer_consumer/subscription"
    And request
        """
        {
          "topics" : [ "trainer_topic" ]
        }
        """
  ...
  Scenario: Getting Subscription
    ...
    Then status 200
    * print response
    * assert response.topics[0] == "trainer_topic"


  Scenario: Getting Subscription records
    Given path "/consumers/my-consumer-group/instances/trainer_consumer/records"
  ...
  Scenario: Getting assigned Assignments
    Given path "/consumers/my-consumer-group/instances/trainer_consumer/assignments"
    And header Accept = "application/vnd.kafka.v2+json"
    When method get
    Then status 200
    * print response
    * assert response.partitions[0].topic == "trainer_topic"
  ...
  Scenario: Deleting consumer
    Given path "/consumers/my-consumer-group/instances/trainer_consumer"
    And header Accept = "application/vnd.kafka.v2+json"
    When method delete
    Then status 204
